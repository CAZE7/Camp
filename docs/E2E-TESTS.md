...
In CI soll der Job `End-to-End (Playwright)` bei jedem Pull Request und vor
jedem Deploy laufen. Workflow-Dateien werden in diesem Restore bewusst nicht
angepasst; falls `.github/workflows/quality.yml` in der Ziel-Branch fehlt, muss
das CI-Gate separat mit passenden `workflows:write`-Rechten übernommen werden.
...
- **Echte Netzbedingungen und Geräteleistung.** Ein Low-End-Android rendert
  ...
  ausgelegt.
- **Pixel-Vergleich nur im UI-Freeze-Gate.** `tests/e2e/visual.spec.ts` prüft
  die Golden Screenshots aus `tests/e2e/visual.spec.ts-snapshots/` (siehe
  `docs/UI-BASELINE.md`). Reine Routing-Regressionen deckt zusätzlich
