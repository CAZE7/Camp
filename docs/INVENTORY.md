...
| Planstatus-Chip                | öffnet die Warn-Zentrale                   | 🆕 UX-Reset | `guided-plan-status`; ohne Hinweise als `role="status"`-Text (kein toter Klick).      |
...
| Detailgrad „Übersichtlich / Mit Werten"                               | schaltet Messwerte der Karten aus/ein   | 🆕 UX-Reset | `detailLevel` (persistiert, Default `detail`), Klasse `planner-detail-*` am Canvas; CSS in `globals.css`. Nutzersteuerbar, nicht zoom-automatisch (`M8-1`). |
