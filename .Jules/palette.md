...
## 2026-05-18 - Visual Feedback for AI Chat Submission
**Learning:** For async operations like sending AI chat messages, relying solely on disabling the submit button is insufficient and leaves the user uncertain if their message is being processed. This can lead to frustration and perceived unresponsiveness.
**Action:** Always provide explicit visual feedback (like a spinning `Loader2` from `lucide-react`) within async submit buttons (e.g., forms, AI prompts) during `isLoading` states, replacing static icons (like `Send`) to assure the user the system is actively working.
