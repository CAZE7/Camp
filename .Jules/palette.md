...
## 2026-05-18 - Segmented Controls vs Tablists
**Learning:** When using standard buttons as a segmented control (like a view mode toggle), using `role="tablist"` on the container and `role="tab"` with `aria-selected` on the buttons is incorrect unless full arrow-key keyboard navigation is implemented. Screen readers expect specific interactions for tabs.
**Action:** For simple segmented toggle buttons without arrow-key navigation, use `role="group"` on the container and `aria-pressed={condition}` on the buttons instead of `role="tab"` and `aria-selected`.
