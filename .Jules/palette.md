...
## 2026-05-18 - Missing aria-expanded on AI Chat Toggle
**Learning:** The floating AI chat component had an open and close button with s, but lacked the  attribute. Without , screen reader users cannot determine whether the chat interface is currently open or closed.
**Action:** Always add  to toggle buttons that control the visibility of chat windows, sidebars, or modal overlays to convey their current state accurately.
## 2026-05-18 - Missing aria-expanded on AI Chat Toggle
**Learning:** The floating AI chat component had an open and close button with `aria-label`s, but lacked the `aria-expanded` attribute. Without `aria-expanded`, screen reader users cannot determine whether the chat interface is currently open or closed.
**Action:** Always add `aria-expanded={isOpenState}` to toggle buttons that control the visibility of chat windows, sidebars, or modal overlays to convey their current state accurately.
