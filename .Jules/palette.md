...
## 2026-05-18 - Missing aria-pressed on Toggleable Buttons
**Learning:** Segmented controls and toggleable buttons like the View Mode, Season, and Pro Mode switches lacked `aria-pressed`. Without this attribute, screen readers perceive them as generic buttons instead of indicating which mode/state is currently active.
**Action:** Always add `aria-pressed={condition}` to toggleable buttons or segmented control buttons that represent states or modes rather than discrete actions.
