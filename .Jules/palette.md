...
## 2026-06-18 - Missing aria-pressed on Toggle Buttons
**Learning:** Segmented control buttons and toggle buttons (like View Mode, Season, or Pro Mode toggles in a dashboard) often lack the `aria-pressed` attribute, leaving screen reader users unaware of which option is currently active.
**Action:** Always ensure that buttons functioning as toggles or segmented choices use the `aria-pressed={condition}` attribute bound to their active state condition.
