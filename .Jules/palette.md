...
## 2026-05-18 - Missing aria-pressed on Segmented Controls
**Learning:** Segmented controls and toggle buttons (like the View Mode, Season, and Pro-Mode toggles in `PlannerDashboard`) relied solely on visual cues (like background color or text changes) to indicate their active state. Without `aria-pressed`, screen readers announce them as generic buttons, leaving users unaware of which option is currently selected.
**Action:** Always add `aria-pressed={condition}` to standalone toggle buttons and segmented control options to ensure their active state is explicitly communicated to assistive technologies.
