...
## 2026-05-18 - Checkbox Label ID Linking
**Learning:** In custom inspector components that map or render multiple checkboxes (like `InverterInspector` and `ConduitInspector`), nested checkboxes or mapped elements must still ensure robust screen reader accessibility. Although nesting `<input>` inside `<label>` works visually, explicitly binding them with `id` and `htmlFor` prevents unpredictable screen reader behavior, ensuring users can accurately interact with them.
**Action:** Always ensure that checkboxes, even when mapped dynamically within labels, explicitly assign `htmlFor` on the label matching a unique `id` on the `<input>`.
