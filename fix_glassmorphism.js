...
(async () => {
  try {
    let content = await fs.readFile('components/Planner.tsx', 'utf8');
    ...
    await fs.writeFile('components/Planner.tsx', content);
  } catch (error) {
    console.error('Error applying glassmorphism fix:', error);
    process.exit(1);
  }
})();
