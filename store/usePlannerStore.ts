...
function withHistory<T extends Partial<PlannerState>>(
  state: PlannerState,
  update: T
    ...
    (set, get) => ({
      ...
      onConnect: (connection) => {
        ...
        if (viewMode === 'water') {
          ...
          const sourceNode =
            nodesMap.get(connection.source || '') || waterNodesMap.get(connection.source || '');
          const targetNode =
            nodesMap.get(connection.target || '') || waterNodesMap.get(connection.target || '');
      ...
      autoWireSystem: () => {
        ...
        if (!result) {
          get().setSystemMessage(
            'Bitte zuerst eine Batterie platzieren, bevor Komponenten verbunden werden.'
          );
          return;
        }
