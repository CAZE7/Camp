...
export type CableEdgeData = {
  ...
  /**
   * Optional Routing V2 polyline. When present it replaces the Bezier/smooth
   * path generated by React Flow so the UI shows the obstacle-aware result.
   */
...
const CableEdge = function ({
  ...
  const [edgePath, labelX, labelY] = useMemo(() => {
    ...
    if (routedPoints && routedPoints.length >= 2) {
      const path = routedPoints
        ...
        .join(' ');
      const mid = routedPoints[Math.floor((routedPoints.length - 1) / 2)];
      return [path, mid.x, mid.y] as const;
    }
  ...
  }, [sourceX, sourceY, sourcePosition, targetX, targetY, targetPosition, isProMode, data?.geometry?.points]);
