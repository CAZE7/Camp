...
function NavigationSection({ viewMode, setViewMode }: { viewMode: 'electric' | 'water'; setViewMode: (mode: 'electric' | 'water') => void }) {
  return (
    <div className="hidden items-center gap-1 lg:flex" role="tablist" aria-label="Planbereich wählen">
      <Button variant={viewMode === 'electric' ? 'default' : 'ghost'} size="sm" role="tab" aria-selected={viewMode === 'electric'} onClick={() => setViewMode('electric')} className="min-h-11">
        <Zap className="mr-1 h-4 w-4" aria-hidden="true" />Elektrik
      </Button>
      <Button variant={viewMode === 'water' ? 'default' : 'ghost'} size="sm" role="tab" aria-selected={viewMode === 'water'} onClick={() => setViewMode('water')} className="min-h-11">
        Wasser
      </Button>
    </div>
  );
...
function ActionsSection({
  ...
  const runAutoWire = () => {
    ...
    setFeedback({ type: hasBattery ? 'success' : 'error', message: hasBattery ? 'Automatische Verkabelung und Dimensionierung abgeschlossen.' : 'Platziere zuerst eine Batterie.' });
  ...
  const onExportImage = useCallback(async () => {
    ...
    try {
      ...
      const bounds = nodes.length > 0 ? getNodesBounds(nodes) : { x: 0, y: 0, width: reactFlowWrapper.clientWidth, height: reactFlowWrapper.clientHeight };
        ...
        {menuOpen && (
          <div role="menu" className="absolute left-0 top-full z-[70] mt-2 w-72 rounded-xl border border-border bg-card p-2 shadow-2xl sm:left-auto sm:right-0">
            ...
            <div className="flex gap-1 px-2 pb-2">
              <Button variant={season === 'summer' ? 'default' : 'ghost'} aria-pressed={season === 'summer'} onClick={() => setSeason('summer')} className="min-h-11 flex-1 gap-1"><Sun className="h-4 w-4" />Sommer</Button>
              <Button variant={season === 'winter' ? 'default' : 'ghost'} aria-pressed={season === 'winter'} onClick={() => setSeason('winter')} className="min-h-11 flex-1 gap-1"><Snowflake className="h-4 w-4" />Winter</Button>
            </div>
...
export function PlannerDashboard() {
  ...
  const warnings = useMemo(() => {
    ...
    nodes.filter((node) => node.type === 'shorePower' && !node.data?.hasRcd).forEach((node) => supplemental.push({
      id: `rcd-${node.id}`, category: 'safety', type: 'critical', title: 'FI-Schutzschalter fehlt',
      focusId: node.id, focusType: 'node',
      message: 'Am Landstromanschluss fehlt ein FI-Schutzschalter mit höchstens 30 mA. Ohne ihn besteht bei einem Fehler Stromschlaggefahr. Lass den 230-V-Schutz von einer Elektrofachkraft ergänzen.',
    }));
    nodes.filter((node) => node.type === 'inverter').forEach((node) => {
      const selectedDeviceIds = new Set<string>(node.data?.concurrentDevices || []);
      const total = nodes.reduce((sum, candidate) => selectedDeviceIds.has(candidate.id) ? sum + (Number(candidate.data?.watts) || 0) : sum, 0);
      if (Number(node.data?.continuousPower) > 0 && total > Number(node.data?.continuousPower)) supplemental.push({
        id: `inverter-overload-${node.id}`, category: 'safety', type: 'critical', title: 'Wechselrichter überlastet', focusId: node.id, focusType: 'node',
    ...
    if (waterWarning) supplemental.push({
      id: 'water-flow-hint', category: 'topology', type: 'info', title: 'Wasserfluss verbessern',
