...
function NavigationSection({ viewMode, setViewMode }: { viewMode: 'electric' | 'water'; setViewMode: (mode: 'electric' | 'water') => void }) {
  return (
    <div className="hidden items-center gap-1 lg:flex" role="group" aria-label="Planbereich wählen">
      <Button variant={viewMode === 'electric' ? 'default' : 'ghost'} size="sm" aria-pressed={viewMode === 'electric'} onClick={() => setViewMode('electric')} className="min-h-11">
        <Zap className="mr-1 h-4 w-4" aria-hidden="true" />Elektrik
      </Button>
      <Button variant={viewMode === 'water' ? 'default' : 'ghost'} size="sm" aria-pressed={viewMode === 'water'} onClick={() => setViewMode('water')} className="min-h-11">
        Wasser
      </Button>
    </div>
  );
        ...
        {menuOpen && (
          <div role="menu" className="absolute left-0 top-full z-[70] mt-2 w-72 rounded-xl border border-border bg-card p-2 shadow-2xl sm:left-auto sm:right-0">
            ...
            <div className="flex gap-1 px-2 pb-2">
              <Button aria-pressed={season === 'summer'} variant={season === 'summer' ? 'default' : 'ghost'} onClick={() => setSeason('summer')} className="min-h-11 flex-1 gap-1"><Sun className="h-4 w-4" />Sommer</Button>
              <Button aria-pressed={season === 'winter'} variant={season === 'winter' ? 'default' : 'ghost'} onClick={() => setSeason('winter')} className="min-h-11 flex-1 gap-1"><Snowflake className="h-4 w-4" />Winter</Button>
            </div>
