...
describe('useDashboardMetrics', () => {
  ...
  it('should return default values for empty inputs', () => {
    const { result } = renderHook(() => useDashboardMetrics(emptyNodes, emptyEdges, 'summer', 0));
  ...
  it('should calculate battery usable capacity correctly (LiFePO4 vs AGM)', () => {
    ...
    const { result: resAGM } = renderHook(() => useDashboardMetrics(nodesAGM, emptyEdges, 'summer', 0));
  ...
  it('should handle seasonal adjustment for consumption', () => {
    const nodes: Node[] = [
      { id: 'b1', type: 'battery', data: { capacity: 100 }, position: { x: 0, y: 0 } },
      {
        id: 'c1',
        type: 'consumer',
        data: { watts: 12, hours: 10, label: 'Autoterm Heizung' },
        position: { x: 0, y: 0 },
      },
    ];

    const { result: resSummer } = renderHook(() => useDashboardMetrics(nodes, emptyEdges, 'summer', 0));
    // (12/12.8) * 10 = 9.375
    expect(resSummer.current.dailyConsumptionAh).toBeCloseTo(9.375, 3);

    const { result: resWinter } = renderHook(() => useDashboardMetrics(nodes, emptyEdges, 'winter', 0));
    // (12/12.8) * 10 * 2 = 18.75
  ...
  describe('Solar Calculations', () => {
    ...
    it('should calculate parallel solar configuration correctly', () => {
      const { result } = renderHook(() => useDashboardMetrics(solarNodes, emptyEdges, 'summer', 0));
    ...
    it('should calculate series solar configuration correctly', () => {
      ...
      const { result } = renderHook(() => useDashboardMetrics(solarNodes, edges, 'summer', 0));
    ...
    it('should apply winter reduction to solar amps', () => {
      const { result } = renderHook(() => useDashboardMetrics(solarNodes, emptyEdges, 'winter', 0));
  ...
  it('should detect direct battery-to-consumer connection', () => {
    ...
    const edges: Edge[] = [{ id: 'e1', source: 'b1', target: 'c1' }];
    const { result } = renderHook(() => useDashboardMetrics(nodes, edges, 'summer', 0));
...
describe('Edge cases and error conditions', () => {
  it('should return "0 Ladeleistung" when there is solar but no charge', () => {
    ...
    const { result } = renderHook(() => useDashboardMetrics(nodes, [], 'summer', 0));
  ...
  it('should calculate metrics when only calculatedSolarWatts is provided', () => {
    ...
    const { result } = renderHook(() => useDashboardMetrics(nodes, [], 'summer', 120));
  ...
  it('should calculate metrics when only calculatedSolarWatts is provided in winter', () => {
    ...
    const { result } = renderHook(() => useDashboardMetrics(nodes, [], 'winter', 120));
  ...
  it('should handle missing nodes gracefully when checking direct connection', () => {
    const edges: Edge[] = [{ id: 'e1', source: 'nonexistent1', target: 'nonexistent2' }];
    const { result } = renderHook(() => useDashboardMetrics([], edges, 'summer', 0));
    expect(result.current.hasDirectBatteryToConsumer).toBe(false);
  });

  it('should handle unmapped node types gracefully', () => {
    const nodes: Node[] = [{ id: 'u1', type: 'unknown_type', data: {}, position: { x: 0, y: 0 } }];
    const { result } = renderHook(() => useDashboardMetrics(nodes, [], 'summer', 0));
    expect(result.current.solarNodesCount).toBe(0);
  });
  ...
  it('should detect direct battery connection when battery is target', () => {
    ...
    const edges: Edge[] = [{ id: 'e1', source: 'c1', target: 'b1' }];
    const { result } = renderHook(() => useDashboardMetrics(nodes, edges, 'summer', 0));
    expect(result.current.hasDirectBatteryToConsumer).toBe(true);
  });
  ...
  it('should re-calculate metrics when edges change', () => {
    ...
    const { result, rerender } = renderHook(({ edges }) => useDashboardMetrics(nodes, edges, 'summer', 0), {
      initialProps: { edges: [] as Edge[] },
    });

    const firstResult = result.current;

    const edges2: Edge[] = [{ id: 'e1', source: 'b1', target: 'c1' }];
  ...
  it('should re-calculate metrics when edge properties change', () => {
    ...
    const edges1: Edge[] = [{ id: 'e1', source: 'b1', target: 'other' }];
    const { result, rerender } = renderHook(({ edges }) => useDashboardMetrics(nodes, edges, 'summer', 0), {
  ...
  it('should detect direct connection from battery to inverter', () => {
    ...
    const edges: Edge[] = [{ id: 'e1', source: 'b1', target: 'i1' }];
    const { result } = renderHook(() => useDashboardMetrics(nodes, edges, 'summer', 0));
    expect(result.current.hasDirectBatteryToConsumer).toBe(true);
  });
  ...
  it('should detect direct connection from inverter to battery', () => {
    ...
    const edges: Edge[] = [{ id: 'e1', source: 'i1', target: 'b1' }];
    const { result } = renderHook(() => useDashboardMetrics(nodes, edges, 'summer', 0));
    expect(result.current.hasDirectBatteryToConsumer).toBe(true);
  });
});
