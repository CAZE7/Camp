...
const ShorePowerNode = function({ id, data, isConnectable, selected }: NodeProps<PlannerNodeData>) {
  ...
  return (
    <div role="group" aria-label={`${data.label || 'Landstromanschluss'}. Komponente im Plan.`} className={`hover:scale-105 transition-all custom-drag-handle bg-white border-2 border-indigo-500 rounded-md p-3 shadow-md w-48 ${selected ? " ring-4 ring-blue-500 shadow-xl" : ""}`}>
      {editingField === 'label' ? (
        ...
        <div className="font-bold mb-2 text-sm text-center cursor-text" onDoubleClick={() => handleDoubleClick('label', data.label || 'Landstromanschluss')}>{data.label || 'Landstromanschluss'}</div>
      )}
