...
const ConsumerNode = function({ id, data, isConnectable, selected }: any) {
  return (
    <div className={`hover:scale-105 transition-all custom-drag-handle bg-white border-2 border-green-500 rounded-md p-3 shadow-md w-48 ${selected ? " ring-4 ring-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.6)]" : ""}`}>
      ...
      <Handle type="target" position={Position.Left} id="plus" isConnectable={isConnectable} className="!w-4 !h-4 !bg-red-600 !border-2 !border-gray-800 !z-10 cursor-pointer" style={{top: '30%'}} />
      <Handle type="target" position={Position.Left} id="minus" isConnectable={isConnectable} className="!w-4 !h-4 !bg-slate-900 !border-2 !border-gray-800 !z-10 cursor-pointer" style={{top: '70%'}} />
