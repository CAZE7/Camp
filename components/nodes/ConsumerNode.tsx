...
const ConsumerNode = function({ id, data, isConnectable, selected }: NodeProps<PlannerNodeData>) {
  ...
  return (
    <div role="group" aria-label={`${data.label || '12-V-Gerät'}. Komponente im Plan.`} className={`hover:scale-105 transition-all custom-drag-handle bg-white border-2 border-green-700 rounded-md p-3 shadow-md w-48 ${selected ? " ring-4 ring-blue-500 shadow-xl" : ""}`}>
      {editingField === 'label' ? (
        <input
          autoFocus
        ...
        <div className="font-bold mb-2 text-sm text-center cursor-text" onDoubleClick={() => handleDoubleClick('label', data.label || 'Verbraucher')}>{data.label || 'Verbraucher'}</div>
      )}
