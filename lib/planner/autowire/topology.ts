 ...
 * @param options  Optionen, u.a. idFactory für stabile IDs in Tests
...
export function buildTopology(
  ...
  const busbar = ensureNode('busbar', 'Main Busbar', DEFAULT_TOPOLOGY_OFFSETS.busbar.x, DEFAULT_TOPOLOGY_OFFSETS.busbar.y);
  const fuseBox = ensureNode('fuse', '12V Sicherungskasten', DEFAULT_TOPOLOGY_OFFSETS.fuseBox.x, DEFAULT_TOPOLOGY_OFFSETS.fuseBox.y, { rating: 100 });
  const shunt = ensureNode('shunt', 'Smart Shunt', DEFAULT_TOPOLOGY_OFFSETS.shunt.x, DEFAULT_TOPOLOGY_OFFSETS.shunt.y);
